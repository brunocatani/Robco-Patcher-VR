...
const char* settingsSectionDiagnostics = "Diagnostics";
...
//	GetPrivateProfileIntA(settingsSection, "iEnableFastTravel", 0, configFile);
//	*/
...
namespace
{
	const char* aliasFile = ".\\Data\\F4se\\Plugins\\RobCo_Patcher\\aliases.ini";
	...
	void LoadAliases()
	{
		...
		std::ifstream input(aliasFile);
		if (!input) {
			return;
		}

		PATCH::RecordFile("aliases", aliasFile);
		...
		while (std::getline(input, line)) {
			...
			PATCH::SetActiveRule("aliases", aliasFile, lineNumber, line);
			...
			if (!PATCH::Aliases().Add(trimmed.substr(0, delimiter), trimmed.substr(delimiter + 1), aliasFile, lineNumber)) {
				PATCH::RecordInvalidRule("aliases", "empty alias name or target");
			}
	...
	void ReportPlacedInstanceData()
	{
		if (!PATCH::ShouldReportInstanceData()) {
			return;
		}

		const auto allForms = RE::TESForm::GetAllForms();
		for (const auto& [key, form] : *allForms.first) {
			if (!form || form->formType != RE::ENUM_FORM_ID::kREFR) {
				continue;
			}

			auto* refr = static_cast<RE::TESObjectREFR*>(form);
			if (!refr || !refr->data.objectReference || !refr->extraList ||
				!refr->extraList->HasType(RE::EXTRA_DATA_TYPE::kInstanceData)) {
				continue;
			}

			const auto baseType = refr->data.objectReference->formType;
			if (baseType == RE::ENUM_FORM_ID::kWEAP || baseType == RE::ENUM_FORM_ID::kARMO) {
				PATCH::RecordMatch("instancedata", formatFormID(form));
			}
		}
	}
	...
	void WriteDiagnosticsReport()
	{
		...
		if (!path) {
			PATCH::RecordWarning("diagnostics", "could not resolve F4SE log directory for report");
			return;
		}
	...
	template <class Fn>
	void RunPatcherCategory(const char* category, Fn&& fn)
	{
		try {
			fn();
		} catch (const std::exception& e) {
			PATCH::RecordInvalidRule(category, e.what());
			logger::error(FMT_STRING("RobCo patch category {} failed: {}"), category, e.what());
		} catch (...) {
			PATCH::RecordInvalidRule(category, "unknown exception");
			logger::error(FMT_STRING("RobCo patch category {} failed with an unknown exception"), category);
		}
	}
		...
		case F4SE::MessagingInterface::kGameDataReady:
			{
				if (static_cast<bool>(a_msg->data)) {
					...
					if (GetPrivateProfileIntA(settingsSection, "iEnableAmmoPatching", 0, configFile)) {
						RunPatcherCategory("ammo", [] { AMMOS::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\ammo\\"); });
					}
					...
					if (GetPrivateProfileIntA(settingsSection, "iEnableRacePatching", 0, configFile)) {
						RunPatcherCategory("race", [] { RACES::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\race\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableNPCPatching", 0, configFile)) {
						RunPatcherCategory("npc", [] { NPCS::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\npc"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableArmorPatching", 0, configFile)) {
						RunPatcherCategory("armor", [] { ARMORS::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\armor\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableFormlistPatching", 0, configFile)) {
						RunPatcherCategory("formlist", [] { FORMLIST::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\formList\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableConstructibleObjectPatching", 0, configFile)) {
						RunPatcherCategory("constructibleobject", [] { COBJ::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\constructibleObject\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableLeveledListPatching", 0, configFile)) {
						RunPatcherCategory("leveledlist", [] { LEVELEDLISTS::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\leveledList\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableObjectModificationPatching", 0, configFile)) {
						RunPatcherCategory("objectmodification", [] { OMOD::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\objectModification\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableMiscPatching", 0, configFile)) {
						RunPatcherCategory("misc", [] { MISC::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\misc\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableIngestiblePatching", 0, configFile)) {
						RunPatcherCategory("ingestible", [] { ALCH::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\ingestible\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableProjectilePatching", 0, configFile)) {
						RunPatcherCategory("projectile", [] { PROJECTILE::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\projectile\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableExplosionPatching", 0, configFile)) {
						RunPatcherCategory("explosion", [] { EXPLOSION::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\explosion\\"); });
					}

					if (GetPrivateProfileIntA(settingsSection, "iEnableOutfitPatching", 0, configFile)) {
						RunPatcherCategory("outfit", [] { OUTFIT::readConfig(".\\Data\\F4se\\Plugins\\RobCo_Patcher\\outfit\\"); });
					}

					if (GetPrivateProfileIntA(settingsSectionFeatures, "iEnableREFRCreateNewInstanceData", 0, configFile)) {
						...
						//						if (GetPrivateProfileIntA(settingsSectionFeatures, "iEnableREFRCleanupWeapons", 0, configFile) && ref && ref->formType == RE::ENUM_FORM_ID::kWEAP) {
						//							if (refr->extraList) {
						...
						//							}
						//						}
						//						if (GetPrivateProfileIntA(settingsSectionFeatures, "iEnableREFRCleanupArmors", 0, configFile) && ref && ref->formType == RE::ENUM_FORM_ID::kARMO) {
						//							if (refr->extraList) {
						//								refr->extraList->RemoveExtra(RE::EXTRA_DATA_TYPE::kInstanceData);
						...
						bool enableWeaponsCleanup = GetPrivateProfileIntA(settingsSectionFeatures, "iEnableREFRCleanupWeapons", 0, configFile);
						bool enableArmorsCleanup = GetPrivateProfileIntA(settingsSectionFeatures, "iEnableREFRCleanupArmors", 0, configFile);
									...
									if (ref) {
										if ( ref->formType == RE::ENUM_FORM_ID::kWEAP) {
											if (refr->extraList && refr->extraList->HasType(RE::EXTRA_DATA_TYPE::kInstanceData)) {
												...
												// VR: SetInstanceData not available in CommonLibF4VR
		...
		case F4SE::MessagingInterface::kPostLoadGame:
			{
				...
				if (GetPrivateProfileIntA(settingsSectionFeatures, "iEnableReCalculateStatsWithSaveLoad", 0, configFile)) {
...
extern "C" DLLEXPORT bool F4SEAPI F4SEPlugin_Query(const F4SE::QueryInterface* a_f4se, F4SE::PluginInfo* a_info)
{
	...
	// FO4VR only has one release (1.2.72) — accept any VR runtime
...
{
	// Init F4SE without auto-logging — we set up the logger ourselves below
	// so we can use Version::PROJECT for the filename and control the log level
	F4SE::Init(a_f4se, false);

	// Set up logger AFTER F4SE::Init so GetSaveFolderName() returns "Fallout4VR"
#ifndef NDEBUG
	...
	*path /= fmt::format(FMT_STRING("{}.log"), Version::PROJECT);
...
#else
	if (GetPrivateProfileIntA(settingsSectionLog, "iEnablelog", 0, configFile)) {
	...
	diagnostics.enableDiagnostics = GetPrivateProfileIntA(settingsSectionDiagnostics, "iEnableDiagnostics", 0, configFile) != 0;
	diagnostics.writePatchReport = GetPrivateProfileIntA(settingsSectionDiagnostics, "iWritePatchReport", 0, configFile) != 0;
	diagnostics.dryRun = GetPrivateProfileIntA(settingsSectionDiagnostics, "iEnableDryRun", 0, configFile) != 0;
	diagnostics.strictConfigValidation = GetPrivateProfileIntA(settingsSectionDiagnostics, "iStrictConfigValidation", 0, configFile) != 0;
	diagnostics.logMatchedRecords = GetPrivateProfileIntA(settingsSectionDiagnostics, "iLogMatchedRecords", 0, configFile) != 0;
	diagnostics.reportInstanceData = GetPrivateProfileIntA(settingsSectionDiagnostics, "iReportInstanceData", 0, configFile) != 0;
	PATCH::ConfigureDiagnostics(diagnostics);
	if (diagnostics.dryRun) {
		logger::info("Diagnostics dry-run enabled; patch rules will be parsed and matched without mutating records.");
	}
