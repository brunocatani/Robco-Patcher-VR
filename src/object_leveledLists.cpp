...
namespace LEVELEDLISTS
{
	...
	void FreeLeveledList(RE::LEVELED_OBJECT* levo, uint32_t arg = 0x3)
	{
		if (!levo) {
			return;
		}
	...
	struct_ll* reserveNewLevL(uint32_t count)
	{
		...
		return (struct_ll*)mm.Allocate(sizeof(RE::LEVELED_OBJECT) * count + sizeof(size_t), 0, 0);
	}
	...
	std::vector<RE::LEVELED_OBJECT> convertLLtoVec(RE::TESLeveledList* levl)
	{
		...
		for (uint32_t i = 0; i < static_cast<uint32_t>(levl->baseListCount); i++) {
	...
	void createLL(RE::TESLeveledList* levl, const std::vector<RE::LEVELED_OBJECT>& levo)
	{
		...
		if (levl->leveledLists) {
			memset(levl->leveledLists, 0, levl->baseListCount * sizeof(RE::LEVELED_OBJECT));
			FreeLeveledList(levl->leveledLists);
		...
		levl->baseListCount = static_cast<int8_t>(size);
	}
		...
		for (const auto& line : tokens) {
			...
			if (!line.objects.empty()) {
				for (const auto& objectstring : line.objects) {
					...
					if (currentform && currentform->formType == RE::ENUM_FORM_ID::kLVLI) {
						...
						if (static_cast<uint32_t>(object->baseListCount) > 120) {
			...
			if (!line.noFilterLL.empty() && toLowerCase(line.noFilterLL) == "true") {
				for (const auto& curobj : objectArray) {
					if (static_cast<uint32_t>(curobj->baseListCount) > 120) {
	...
	{
		if (!curobj || ShouldSkipPatch("leveledlistcontainer", curobj)) {
			return;
		}
		...
		if (!line.removedContainerObjects.empty()) {
			for (const auto& objectToAdd : line.removedContainerObjects) {
				...
				if (addForm) {
					VRCompat::RemoveObject(curobj, addForm);
